...
def load_request():
    ...
    # Configs
    configs = yaml.load(open(os.environ.get('YAML_FILE')),
    ...
    if request_type == 'text':
        ...
        content = configs['text']
        request = StructuredTextRequest(texts=[Text(content=content)] * 2,
                                        params=params)
    ...
    if response_type not in [
            'annotations', 'audio', 'classification', 'texts'
    ]:
    ...
    return url, headers, request, params, response_type,\
        content, text, trial_num, thread_num
...
class TestELG(unittest.TestCase):
    url, headers, request, params, response_type,\
        content, text, trial_num, thread_num = load_request()
    ...
    def test_res_type(self):
        ...
        if isinstance(self.request, AudioRequest):
            res = requests.post(self.url,
                                ...
                                files=audio_req_files(self.request, self.text))
        else:
            res = requests.post(self.url,
                                headers=self.headers,
        ...
        for i in range(times):
            if isinstance(self.request, AudioRequest):
                requests.post(self.url,
                              headers=self.headers,
                              files=audio_req_files(self.request))
            else:
                requests.post(self.url,
                              headers=self.headers,
                              json=self.request.dict())
    ...
    def test_emp_req(self):
        """
        Test the service with empty request, it should
            return empty response or failure or error
        """
        ...
        if isinstance(self.request, AudioRequest):
            res = requests.post(self.url,
                                headers=self.headers,
                                files=audio_req_files(empty_req))
        else:
            res = requests.post(self.url,
                                headers=self.headers,
                                json=empty_req.dict())
    ...
    def test_large_req_mix(self):
        ...
        API service should still work and also return warning
        on the part of texts that was failed to parse"""

        if isinstance(self.request, AudioRequest) or isinstance(
                self.request, TextRequest):
            return
        large_text_content = " ".join([self.content] * 100)
        large_mix_req = StructuredTextRequest(
            texts=[Text(content=self.content)] * 2 +
            [Text(content=large_text_content)],
            params=self.params)
        res = requests.post(self.url,
                            headers=self.headers,
                            json=large_mix_req.dict())

        assert res is not None
        assert res.status_code == 200
        res = res.json()
        assert 'response' in res
        self.assertIn(res['response']['type'],
                      ['texts', 'annotations', 'classification'],
                      msg='Wrong type returns')
        if res['response'][
                'type'] == 'texts':  # only tnpp and other annotation tool APIs needs
            warnings = res['response']['warnings']
            self.assertIsInstance(warnings, list,
                                  'given object is not List type')
    ...
    def test_inv_param(self):
        ...
        If Failure return, test if it is compatible with ELG
        """
    ...
    def test_load(self):
        ...
        for i in range(t_num):
            if isinstance(self.request, AudioRequest):
                t_list.append(
                    ReqThread(self.url, self.headers,
                              audio_req_files(self.request), times, True))
            else:
                t_list.append(
                    ReqThread(self.url, self.headers, self.request.dict(),
                              times))
            t_list[i].start()
        ...
        for t in t_list:
            t.join()
        et = time.time()
        print("Multithreading: Average response time: %.2fs" %
              ((et - st) / (t_num * times)))
